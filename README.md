...
- Pick a random vector $r_w$ = $\left\{+1,-1\right\}^{|d|}$ made of +1,-1 drawn uniformly at random (as many as the length |d| of document vectors)
...
- For each word w pick a random vector $r_w$ = $\left\{+1,-1\right\}^m$ of length m made of +1,-1
