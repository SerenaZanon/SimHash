...
for dataset, dataset_name in zip(datasets, datasets_name):
    ...
    for values in possible_combinations:
        ...
        pairs_signatures = group_signatures.flatMap(lambda block: list(combinations(block[1], 2)))

        num_pairs_signatures = pairs_signatures.count()
        print("\n\nComputing number of all pairs done!\n\n")

        new_num_pairs_signatures = pairs_signatures.distinct().count()
        print("\n\nComputing distinct number of pairs done!\n\n")

        dataset_info.append([values[0], values[1], num_pairs_signatures, new_num_pairs_signatures])

    # Save dataset info to CSV
    to_csv(dataset_info, dataset_name + "_count" + ".csv")
